...
{   
    ...
    if ( certif )
    {
        net->alert = (char**)malloc(sizeof(char*) * 5);
        net->alert[0] = strdup("Connected with encryption:");
        net->alert[1] = strdup(SSL_get_cipher(net->ssl.ssl));
        net->alert[2] = strdup("- SSL Certifificate Issuer -");
        net->alert[3] = X509_NAME_oneline(X509_get_issuer_name(certif), 0, 0);
        net->alert[4] = 0;
        X509_free(certif);
    }
    else
